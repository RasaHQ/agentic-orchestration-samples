...
def generate_appointment_slots(start_date, end_date,
...
def query_available_appointments(
    ...
    if not user_availability_end_date or user_availability_end_date.lower() == "any":
        ...
        try:
            start_dt = datetime.strptime(user_availability_start_date, "%d/%m/%Y")
            user_availability_end_date = (start_dt + timedelta(days=14)).strftime("%d/%m/%Y")
        except ValueError:
            user_availability_end_date = (datetime.now() + timedelta(days=14)).strftime("%d/%m/%Y")
    ...
    try:
        # Parse dates
        start_datetime = datetime.strptime(user_availability_start_date, "%d/%m/%Y")
        end_datetime = datetime.strptime(user_availability_end_date, "%d/%m/%Y")
        ...
        result = {
            ...
            "search_criteria": {
                ...
                "preferred_doctor": preferred_doctor if preferred_doctor and preferred_doctor.lower() != "any" else "Any doctor",
                "excluded_dates": [date.strftime("%d/%m/%Y") for date in excluded_dates] if excluded_dates else []
            }
        }
        ...
        if available_slots:
            if preferred_doctor and preferred_doctor.lower() != "any":
                result[
                    "message"] = f"Found {len(available_slots)} available appointment slots with Dr. {preferred_doctor}"
            else:
                result["message"] = f"Found {len(available_slots)} available appointment slots"
        else:
            result[
                "message"] = "No appointments are available in your specified time range. Please try a different date or time."
