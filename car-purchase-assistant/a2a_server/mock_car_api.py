...
class MockCarSearchAPI:
    ...
    def search_cars(
        ...
        Args:
            ...
            price_range (tuple, optional): A tuple representing the min and max price (e.g., (25000, 30000)).
        ...
        """
        print(
            f"DEBUG: Mock Car Search API called with: type='{car_type}', price_range={price_range}, new_or_used='{new_or_used}', model='{model_name}'"
        )

        # Set default price range if not provided
        if price_range is None:
            price_range = (10000, 60000)  # Broad default range
        min_price, max_price = price_range

        found_car = None
        for car in self.cars_data:
            # Check if this car matches all provided criteria
            matches = True

            # Check model name if specified (partial matching for flexibility)
            if model_name:
            ...
            # Check price range
            if matches and not (min_price <= car["price"] <= max_price):
                matches = False
...
if __name__ == "__main__":
    ...
    search_result_1 = car_api.search_cars(
        car_type="compact SUV", price_range=(25000, 30000), new_or_used="new"
    )
    ...
    search_result_2 = car_api.search_cars(
        car_type="sedan", price_range=(20000, 27000), new_or_used="used"
    )
    ...
    search_result_3 = car_api.search_cars(
        car_type="truck", price_range=(10000, 20000), new_or_used="new"
    )
