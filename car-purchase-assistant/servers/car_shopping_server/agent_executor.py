...
from a2a.types import (
    TaskState,
    UnsupportedOperationError, TextPart
)
...
conversations = [
    ...
    ConversationUpdate(
        TaskState.working,
        "Let me look up for a car deal for you",
    ),
    ConversationUpdate(
        TaskState.working,
        "I am expanding the search",
    ),
    ConversationUpdate(
        TaskState.working,
        "Fetching data from car dealers",
    ),
    ConversationUpdate(
        TaskState.completed,
...
class CarShoppingAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        for part in context.message.parts:
            ...
            if isinstance(part.root, TextPart) and part.root.text == "hello":
                await updater.update_status(
                    TaskState.completed
                )
                return

        async for conversation_item in get_next_update():
            ...
            await updater.update_status(
                conversation_item.task_state,
                message,
                metadata=metadata,
            )
            logger.info(f"Sending message {conversation_item.message}")
            await asyncio.sleep(conversation_item.latency)
